...
if ! buildah inspect --type container "${container}" &>/dev/null; then
    container=$(buildah from --name "${container}" docker.io/library/debian:12.13-slim)
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=$(printf "%s:${IMAGETAG:-latest} " "${images[@]}") docker.io/library/nginx:1.29.6-alpine" \
    "${container}"
# Commit the image
buildah commit "${container}" "${repobase}/${reponame}"
